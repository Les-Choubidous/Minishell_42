...
/*   Updated: 2025/01/17 17:58:47 by memotyle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int execute_builtin(int *fd_pipes, int pos, t_commands *command, t_data *data)
{
    ...
    if (dup2(fd_pipes[pos], STDIN_FILENO) == -1 || dup2(fd_pipes[pos + 3], STDOUT_FILENO) == -1)
	{
        perror("dup2 error");
        close(original_fd[0]);
        close(original_fd[1]);
    ...
    if (dup2(original_fd[0], STDIN_FILENO) == -1 || dup2(original_fd[1], STDOUT_FILENO) == -1)
	{
        close(original_fd[0]);
        close(original_fd[1]);
